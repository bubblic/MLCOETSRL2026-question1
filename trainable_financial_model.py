...
class TrainableFinancialModel(tf.Module):
    def __init__(self):
        ...
        self.depreciation_rate = tf.Variable(0.10, name="dep_rate")
        self.min_cash_ratio = tf.Variable(
            0.05, name="min_cash_ratio"
        )  # Cash as % of Sales

        # These are set constant for now
...
def train_model(model, actual_assets, sales_data, cost_data, epochs=100):
    ...
    print(
        f"{'Epoch':<10} | {'Loss':<10} | {'Cash Ratio':<10} | {'Tax Rate':<10} | {'Depr Rate':<10}"
    )
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0:
            print(
                f"{epoch:<10} | {loss.numpy():<10.2f} | {model.min_cash_ratio.numpy():<10.2f} | {model.tax_rate.numpy():<10.2f} | {model.depreciation_rate.numpy():<10.2f}"
            )
