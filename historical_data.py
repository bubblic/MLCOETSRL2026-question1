...
def get_apple_historical_data():
    ...
    depreciation = np.array(
        [10903000000, 12547000000, 11056000000, 11284000000,
         11104000000, 11519000000, 11445000000, 11698000000],
        dtype=np.float64,
    )
    opex = np.array(
        [30941000000, 34462000000, 38668000000, 43887000000,
         51345000000, 54847000000, 57467000000, 62151000000],
        dtype=np.float64,
    )
    net_income = np.array(
        [59531000000, 55256000000, 57411000000, 94680000000,
         99803000000, 96995000000, 93736000000, 1.1201e11],
        dtype=np.float64,
    )
    tax = np.array(
        [13372000000, 10481000000, 9680000000, 14527000000,
         19300000000, 16741000000, 29749000000, 20719000000],
        dtype=np.float64,
    )
    ...
    inventory_plus_one = np.array(
        [4855000000, 3956000000, 4106000000, 4061000000, 6580000000,
         4946000000, 6331000000, 7286000000, 5718000000],
        dtype=np.float64,
    )
    nca = np.array(
        [2.34386e11, 1.75697e11, 1.80175e11, 2.16166e11,
         2.1735e11, 2.09017e11, 2.11993e11, 2.11284e11],
        dtype=np.float64,
    )
    accounts_receivable = np.array(
        [48995000000, 45804000000, 37445000000, 51506000000,
         60932000000, 60985000000, 66243000000, 72957000000],
        dtype=np.float64,
    )
    accounts_payable = np.array(
        [55888000000, 46236000000, 42296000000, 54763000000,
         64115000000, 62611000000, 68960000000, 69860000000],
        dtype=np.float64,
    )
    advance_payments_purchases = np.array(
        [12087000000, 12352000000, 11264000000, 14111000000,
         21223000000, 14695000000, 14287000000, 14585000000],
        dtype=np.float64,
    )
    advance_payments_sales = np.array(
        [5966000000, 5522000000, 6643000000, 7612000000,
         7912000000, 8061000000, 8249000000, 9055000000],
        dtype=np.float64,
    )
    cash = np.array(
        [25913000000, 48844000000, 38016000000, 34940000000,
         23646000000, 29965000000, 29943000000, 35934000000],
        dtype=np.float64,
    )
    ims = np.array(
        [40388000000, 51713000000, 52927000000, 27699000000,
         24658000000, 31590000000, 35228000000, 18763000000],
        dtype=np.float64,
    )
    current_liabilities = np.array(
        [55012000000, 53960000000, 56453000000, 63106000000,
         81955000000, 74636000000, 99183000000, 86716000000],
        dtype=np.float64,
    )
    non_current_liabilities = np.array(
        [1.41712e11, 1.4231e11, 1.53157e11, 1.62431e11,
         1.48101e11, 1.45129e11, 1.31638e11, 1.19877e11],
        dtype=np.float64,
    )
    equity = np.array(
        [1.07147e11, 90488000000, 65339000000, 63090000000,
         50672000000, 62146000000, 56950000000, 73733000000],
        dtype=np.float64,
    )
    st_debt = np.array(
        [45291000000, 43700000000, 47680000000, 53493000000,
         70827000000, 64814000000, 88271000000, 74366000000],
        dtype=np.float64,
    )

    # --- Cash Flow ---
    dividends = np.array(
        [13712000000, 14119000000, 14081000000, 14467000000,
         14841000000, 15025000000, 15234000000, 15421000000],
        dtype=np.float64,
    )
    stock_buyback = np.array(
        [72738000000, 66897000000, 72358000000, 85971000000,
         89402000000, 77550000000, 94949000000, 90711000000],
        dtype=np.float64,
    )
