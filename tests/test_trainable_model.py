...
Run the script by:
python -m pytest -q tests/test_trainable_model.py
"""
...
from financial_forecast.models import trainable_model as module
...
def test_financial_state_from_dict_filters_unknown_keys():
    ...
    state = FinancialState.from_dict(data)
...
def test_forecast_step_outputs_finite_and_identity_close(
    ...
    assert isinstance(predicted, FinancialState)
...
def test_to_dict_roundtrip(model, state_dict, econ_inputs):
    ...
    restored = FinancialState.from_dict(as_dict)
