...
via :meth:`BaseFinancialModel.prepare` /
:meth:`TrainableFinancialModel.train`, runs the trajectory simulator,
computes one-step-ahead historical fit, and plots results.
...
class ForecastPipeline:
    ...
    constructing the pipeline.  The pipeline reads the model's initial
    state and forecast drivers, runs the trajectory simulator (which
    calls ``model.forecast_step_compiled`` as the evolution function),
    ...
    def __init__(self, model, show_plot: bool = False):
        self.model = model
    ...
    def _run_trajectory_forecast(self) -> Dict[str, tf.Tensor]:
        ...
        return model.trajectory_simulator.run(
            ...
            model._sales_forecast,
            model._cum_inf_forecast,
            model._forecast_years,
        )
    ...
    def _plot_results(self, trajectories, historical_fit, fit_years) -> None:
        ...
        plot_historical_and_forecast(
            ...
            forecast_years=model._forecast_years,
            historical_data=hist_data,
            forecast_trajectories=trajectories,
            amount_scale=scale,
            sales_hist_usd=d["sales"],
            sales_forecast_usd=model._sales_forecast_usd,
