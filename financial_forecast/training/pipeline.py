...
This module wires together data loading, model training, Monte Carlo
forecasting, and result plotting into a single ``run_training_and_forecast``
entry point.  All financial logic is delegated to the model and helper
modules; this file only handles orchestration and data plumbing.
"""
...
from financial_forecast.inference.plotting import plot_historical_and_forecast, plot_opex_fit_with_aleatoric_noise
...
def _validate_historical_data(data: Mapping[str, tf.Tensor]) -> None:
    """Validate that all required keys are present in historical data.

    Args:
        data: Mapping of historical financial series keyed by canonical
            name (e.g. ``"sales"``, ``"cogs"``).

    Raises:
        ValueError: If any required key is missing from *data*.
    """
    required_keys = {
        "sales",
        "purchases",
...
def run_training_and_forecast(
    ...
    Args:
        historical_data: Mapping of historical financial series (USD).
        sales_forecast_usd: Forecasted annual sales in USD.
        inflation_forecast: Forecasted annual inflation rates.
        use_trained_parameters: If ``True``, load parameters from disk
            instead of training.
        parameters_path: File path for saving/loading trained parameters.
        use_inflation: Whether to incorporate inflation in the model.
        include_tax_anomalies: Whether to include one-time tax anomaly
            data during training.
        simple_policy_epochs: Number of optimisation epochs for simple
            policy training.
        structural_epochs: Number of optimisation epochs for structural
            parameter training.
        monte_carlo_samples: Number of Monte Carlo samples for the
            forecast simulation.

    Raises:
        ValueError: If epoch or sample counts are less than 1, or if
            forecast inputs have invalid shapes.
    """
