...
def plot_opex_fit_with_aleatoric_noise(
    ...
    Args:
        model: Trained financial model exposing OpEx variational parameters
            (``q_var_opex_loc``, ``q_base_opex_loc``, etc.).
        historical_years: 1-D tensor of year indices for x-axis labelling.
        historical_sales_bil: Historical sales in billions (scaled).
        historical_opex_bil: Historical OpEx in billions (scaled).
        historical_inflation: Historical per-year inflation rates; if
            ``None``, zeros are used.
