...
class FinancialStatementExtractor(BasePdfExtractor):
    ...
    def _extract_one_pdf(self, pdf_path: Path, output_dir: Path):
        ...
        for query in self.queries:
            ...
            supplementary_pages = self._select_pages(
                pages, supplementary_query, is_financial_statement=False,
            )
            print(f"Supplementary pages: {supplementary_pages}")

            supplementary_extraction = self._extract_supplementary(
                query, primary_extraction, supplementary_pages, pages,
            )
            ...
            slug = (
                re.sub(r"[^a-z0-9]+", "-", query.lower()).strip("-")
                or "query"
            )
            self._write_json(output_dir, pdf_path, slug, result)
    ...
    def _extract_table(
        ...
        prompt = (
            ...
            f"in a nice tabular form (in English) from the following "
            f"pages data.\nPages:\n{pages_text}"
        )
    ...
    def _extract_supplementary(
        ...
        primary_context = json.dumps(
            primary_extraction, ensure_ascii=False, indent=2,
        )

        for start in range(
            0, len(page_numbers), SUPPLEMENTARY_EXTRACTION_MAX_PAGES,
        ):
            chunk = page_numbers[
    ...
    def _build_supplementary_page_query(
        query: str, primary_extraction: dict,
    ) -> str:
        """Build query for locating supplementary disclosure pages."""
        primary_context = json.dumps(
            primary_extraction, ensure_ascii=False,
        )
        return (
            f"Supplementary disclosures, breakdowns, expansions, and "
            f"note tables for {query}. Use this extracted primary "
            f"statement as context to infer relevant line items and "
