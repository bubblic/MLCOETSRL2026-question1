...
This module implements a Bayesian/VI financial model that extends tf.Module
directly. Unlike the simple and trainable models in this package, the Bayesian
model does NOT share the BaseFinancialModel interface because:

- It uses raw dicts instead of FinancialState dataclasses for state.
- It has completely different parameters (TransformedVariable with bijectors).
- Its forecast_step has a different signature (use_mean_opex, sampled params).

The model learns a variational posterior over operating expense parameters
(variable OpEx percentage and baseline OpEx) using the reparameterization
trick, while simultaneously fitting deterministic policy parameters via
gradient descent. Structural parameters (interest rates, maturity, financing
mix) are trained in a second stage using state-transition losses.
"""
...
from financial_forecast.training.io_utils import TRAINING_RESULTS_DIR, _get_training_results_path
...
class BayesianFinancialModel(tf.Module):
    """Bayesian financial model using variational inference for OpEx estimation.

    This model implements a full financial forecasting pipeline based on the
    Pareja (2009) Cash Budget construction. It combines deterministic policy
    parameters (asset growth, depreciation, working capital ratios, etc.) with
    Bayesian variational inference for operating expenses.

    The model is structured in three layers:
        1. **Policy parameters** (deterministic): Trained via simple regression
           against historical ratios (e.g., asset growth, depreciation rate,
           working capital percentages, cost ratio, dividend payout).
        2. **Bayesian OpEx parameters**: Learned via variational inference with
           a Normal posterior and wide Gaussian prior, using the ELBO objective.
        3. **Structural parameters**: Trained via gradient descent on
           state-transition losses (interest rates, debt maturity, equity
           financing mix).
    ...
    def save_parameters(self, path):
        ...
        Serializes every learnable parameter (policy, Bayesian, and structural)
        along with the base_year metadata to a NumPy .npz archive.
    ...
    def load_parameters(self, path):
        ...
        Restores all learnable parameters from a previously saved archive.
    ...
    def sample_opex_params(self):
        ...
        Uses the reparameterization trick to draw differentiable samples
        from the learned Normal posteriors for the variable OpEx percentage
        and baseline OpEx amount.

        Returns:
            A tuple ``(var_opex_sample, base_opex_sample)`` of scalar
            tf.Tensor values drawn from the respective posteriors.
        """
    ...
    def get_opex_kl_divergence(self):
        """Compute the KL divergence between the OpEx posterior and prior.

        Calculates ``KL(q || p)`` for both the variable OpEx percentage and
        baseline OpEx parameters. The priors are intentionally very wide
        Gaussians (scale=1e10) centered at zero, approximating uniform
        distributions so that the data dominates the posterior.

        Returns:
            A scalar tf.Tensor with the summed KL divergence.
        """
    ...
    def train_simple_policies(
        ...
        alongside the variational OpEx parameters in a single Adam loop. Each
        loss component is normalized by its historical standard deviation (when
        ``loss_scale_mode="std"``) to ensure balanced gradients across
        ...
        Args:
            ...
            historical_eff_st_debt: 1-D array-like of effective short-term debt.
            historical_tax_onetime_payments: Optional 1-D array-like of
                one-time tax payments. Defaults to zeros.
            historical_inflation: Optional 1-D array-like of annual inflation
                rates. Defaults to zeros.
            historical_years: Optional 1-D array-like of fiscal years
                corresponding to each observation. If None, sequential indices
                starting from 0 are used.
            learning_rate: Adam optimizer learning rate. Defaults to 0.001.
            epochs: Number of training iterations. Defaults to 25000.
            plot_vi: Whether to save diagnostic plots. Defaults to True.
            plot_every: Logging and history recording interval. Defaults to
                1000.
            show_plot: Whether to display plots interactively (vs. only saving
                to disk). Defaults to False.
            prior_strength_asset_maintain: Strength of the quadratic prior
                pulling asset_maintain toward 1.0. Defaults to 1.0.
            loss_scale_mode: ``"std"`` normalizes each loss by the historical
                standard deviation; ``"none"`` uses unit scaling.
        """
    ...
    def train_structural_parameters(
        ...
        prediction errors. For each consecutive pair of historical years, the
        model runs ``forecast_step`` (with deterministic mean OpEx) from the
        observed state at time *t* and compares the predicted state at *t+1*
        ...
        Args:
            ...
            historical_opex: 1-D array-like of operating expenses.
            historical_tax: 1-D array-like of tax payments.
            historical_effective_st_debt: 1-D array-like of effective
                short-term debt.
            historical_current_lt_debt: 1-D array-like of current portion of
                long-term debt.
            historical_non_current_liabilities: 1-D array-like of non-current
                liabilities.
            historical_interest_payment: 1-D array-like of interest payments.
                May contain NaN/Inf for missing observations.
            historical_ms_return: 1-D array-like of returns from market
                securities investment.
            historical_equity: 1-D array-like of stockholders' equity.
            historical_tax_onetime_payments: Optional 1-D array-like of
                one-time tax payments. Defaults to zeros.
            historical_inflation: Optional 1-D array-like of annual inflation
                rates. Defaults to zeros.
            historical_years: Optional 1-D array-like of fiscal years. If
                None, years are inferred from ``base_year``.
            learning_rate: Adam optimizer learning rate. Defaults to 0.001.
            epochs: Number of training iterations. Defaults to 20000.
            plot_every: History recording interval. Defaults to 1000.
            gradient_clip_norm: Maximum gradient norm for clipping. Set to
                None or 0 to disable. Defaults to 5.0.
            show_plot: Whether to display plots interactively. Defaults to
                False.
            loss_scale_mode: ``"std"`` normalizes each loss by the historical
                standard deviation; ``"none"`` uses unit scaling.
        """
    ...
    def forecast_step(
        ...
        """Advance the financial state by one period using the Cash Budget model.

        Implements the Pareja (2009) Cash Budget construction: given the
        balance-sheet state at *t-1* and exogenous inputs at *t* (sales, year,
        cumulative inflation), this method computes every line item of the
        income statement, liquidity budget, and balance sheet at *t*.

        The operating expense can be computed deterministically (posterior
        mean) or stochastically (sampled structural params plus annual noise),
        controlled by ``use_mean_opex`` and the optional sampled parameters.

        Args:
            state: A dict mapping balance-sheet keys to scalar tensors
                representing the financial state at *t-1*. Required keys:
                ...
                ``current_lt_debt``, ``non_current_liabilities``, ``equity``,
                ``net_income``, ``dividends``.
            inputs: A dict of exogenous inputs for period *t*. Required keys:
                ``sales_t``, ``year``, ``cum_inflation``. Optional:
                ``tax_onetime_payment``.
            use_mean_opex: If True, use the posterior mean for OpEx (no
                sampling or noise). Suitable for structural training and
                deterministic forecasts. Defaults to False.
            sampled_var_opex: Pre-sampled variable OpEx percentage. If None
                and ``use_mean_opex`` is False, falls back to the posterior
                mean.
            sampled_base_opex: Pre-sampled baseline OpEx. If None and
                ``use_mean_opex`` is False, falls back to the posterior mean.

        Returns:
            A dict mapping output keys to scalar tensors for the predicted
            state at *t*, including balance-sheet items, income-statement
            components, and diagnostic quantities (``liquidity_check``,
            ``check``).
        """
