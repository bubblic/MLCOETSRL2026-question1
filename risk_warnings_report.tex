...
\tikzset{
  ...
  output/.style={ellipse, minimum width=2.6cm, minimum height=1cm,
    ...
    font=\small\bfseries},
  annotation/.style={font=\scriptsize\itshape, text=codegray},
...
\noindent The extraction engine can detect such escalations by running
Stage~2 extraction on consecutive annual reports for the same company and
comparing the structured output. For example, a \texttt{debt\_covenants}
result that returns \texttt{compliance\_status: ``in compliance''} in year
$t-1$ but \texttt{compliance\_status: ``in breach''} in year $t$ represents
a programmatically detectable escalation.
