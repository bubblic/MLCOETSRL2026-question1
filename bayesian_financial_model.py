...
class BayesianFinancialModel:
        ...
        return tfd.JointDistributionNamed(
            {
                ...
                "depreciation_rate": tfd.Sample(
                    tfd.Uniform(low=tf.cast(0.0, tf.float64), high=0.2)
                ),
    ...
    def get_likelihood(
        ...
        likelihoods = [
            tfd.Normal(
                loc=pred_delta_nca,
                scale=self.noise_scale * tf.math.reduce_std(delta_nca_true),
            ),
            tfd.Normal(
                loc=pred_depr,
                scale=self.noise_scale * tf.math.reduce_std(historical_depr),
            ),
            tfd.Normal(
                loc=pred_ar, scale=self.noise_scale * tf.math.reduce_std(historical_ar)
            ),
            tfd.Normal(
                loc=pred_tax,
                scale=self.noise_scale * tf.math.reduce_std(historical_tax),
            ),
        ]
    ...
    def fit(self, sales, nca, depr, ar, tax, ni, epochs=1000):
        ...
        def target_log_prob(**params):
            ...
            likelihood_log_prob = self.get_likelihood(
                params, sales, nca, depr, ar, tax, ni
            )
