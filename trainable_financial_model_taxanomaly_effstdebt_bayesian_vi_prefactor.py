...
class TrainableFinancialModel(tf.Module):
    ...
    def train_structural_parameters(
        ...
        eff_st_t = tf.convert_to_tensor(historical_effective_st_debt, dtype=tf.float64)
        curr_lt_t = tf.convert_to_tensor(historical_current_lt_debt, dtype=tf.float64)
...
def run_training_and_forecast(
    ...
    state = {
        ...
        "effective_st_debt": tf.constant(
            effective_st_debt_hist_bil[-2], dtype=tf.float64
        ),
        "current_lt_debt": tf.constant(current_lt_debt_hist_bil[-2], dtype=tf.float64),
    ...
    inflation_forecast = np.array(
        [inflation_hist[-1], 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03],
        dtype=np.float64,
    )
    ...
    for t in range(n_hist_points - 1):  # t = 0..6, predicting index t+1
        ...
        state_t = {
            ...
            "effective_st_debt": tf.constant(
                effective_st_debt_hist_bil[t], dtype=tf.float64
            ),
            "current_lt_debt": tf.constant(
                current_lt_debt_hist_bil[t], dtype=tf.float64
            ),
            "non_current_liabilities": tf.constant(
