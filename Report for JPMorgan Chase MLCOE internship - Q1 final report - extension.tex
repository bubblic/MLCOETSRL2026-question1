...
(Note: while the model avoids the traditional balance-sheet plug, Purchases is computed from the inventory identity $\text{COGS} + \Delta\text{Inv}$, which functions as a derived quantity that ensures inventory consistency. Unlike a balance-sheet plug, this is a structural accounting identity, not a residual forced to reconcile Assets $=$ Liabilities $+$ Equity.)
...
The equity financing proportion is trained from historical data, implicitly capturing the firm's revealed preference between debt and equity. This is consistent with the pecking order hypothesis (Myers \& Majluf, 1984), which predicts firms prefer internal funds, then debt, then equity---resulting in a low and relatively stable \%EF for profitable firms. The trained \%EF thus reflects the firm's historical position on the pecking order rather than an arbitrary split.
...
\textbf{Derivation of historical \%MSReturn.}
...
When Yahoo Finance does not provide Interest Expense separately (as is
...
\item
  \%MSReturn : Effective percentage return on the investment in market
...
This flat-rate assumption is a first-order simplification. In practice, borrowing costs depend on the firm's leverage and credit quality (Merton, 1974). As the forecast shows rising debt levels, a more complete model would incorporate credit spread widening---a natural extension that would link the cost of debt to the evolving debt-to-equity ratio. The current approach is conservative: it understates the cost of increased leverage, producing optimistic net income projections for high-debt scenarios.
...
\textbf{run\_simple\_model\_forecast.py}, which composes
...
The following is the output of the code when run with: \texttt{python run\_simple\_model\_forecast.py} :
...
The code has been implemented in \textbf{run\_trainable\_model\_forecast\_simple\_policies.py},
...
running \textbf{run\_trainable\_model\_forecast\_simple\_policies.py}:
...
{\sloppy We implemented the updated Bayesian model in \textbf{run\_trainable\_model\_forecast\_adv\_policies\_w\_bayesianopex.py},
...
Then, we account for interest payments and ST investment returns
(derived as $\text{EBT} - \text{EBIT} + \text{InterestExpense}$ from the
income statement, see \eqref{eq:ms_return_derivation}), which
...
Script: \textbf{run\_tax\_anomaly\_extraction.py} (using \texttt{TaxAnomalyExtractor} from \texttt{financial\_forecast/extraction/})
...
run
\textbf{run\_trainable\_model\_forecast\_adv\_policies\_w\_bayesianopex\_taxanomaly.py},
...
sent to our reasoning model via \textbf{run\_recommendation\_to\_ceo.py}.
...
\textbf{Modular Architecture and Forecast Report Pipeline}
...
\textbf{\ul{run\_statement\_extraction.py}} (using \texttt{FinancialStatementExtractor} from \texttt{financial\_forecast/extraction/})
...
Run multi-run normalization via \textbf{run\_statement\_normalization\_multi.py},
...
\textbf{run\_ratio\_calculation.py} (using \texttt{RatioCalculator} from
