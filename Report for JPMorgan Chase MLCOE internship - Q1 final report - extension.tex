...
\item
  ...
  \end{itemize}
\item
  TL\textsubscript{baseline}, $\alpha_{\text{TL}}$, $\beta_{\text{TL}}$ :
...
\item
  $\alpha_{\text{Cash}}$, $\beta_{\text{Cash}}$ : Logit-linear trend
  parameters for the cash share of total liquidity:
...
\item
  EffSTDebt\textsubscript{baseline} and \%STDebt : Baseline and
...
\item
  $\alpha_{\text{EF}}$, $\beta_{\text{EF}}$ : Logit-linear parameters
  for the equity financing mix, $\text{\%EF}_t = \sigma(\alpha_{\text{EF}}
  + \beta_{\text{EF}} \cdot t)$, where \%EF is the fraction of long-term
  financing raised as equity rather than debt
\item
  \%PR : Target dividend payout ratio (fraction of net income) in the
  Lintner partial-adjustment model
\item
  Adj.\ Speed : Lintner partial-adjustment speed controlling how
  quickly actual dividends converge to the target payout
  $\text{\%PR} \cdot \text{NI}_{t-1}$
\item
  BB\textsubscript{baseline} and BB\textsubscript{ratio} : Fixed
  baseline and depreciation-proportional parameters for stock buyback
