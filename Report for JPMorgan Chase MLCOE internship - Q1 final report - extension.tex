...
\caption{Ten-year forecast from the trained advanced-policy model: Balance Sheet (top), Income Statement (middle), and Cash Budget (bottom).}
...
\caption{Ten-year Bayesian forecast with 95\% Monte Carlo confidence intervals: Balance Sheet (top), Income Statement (middle), and Cash Budget (bottom).}
...
\caption{Final ten-year forecast with tax anomaly adjustment, Bayesian OpEx, and advanced policies: Balance Sheet, Income Statement, and Cash Budget.}
...
\caption{LLM-only balance sheet forecast for Apple using DeepSeek\,V3.2, showing that the model fails to maintain the accounting identity $A = L + E$.}
...
\caption{Excerpts from Apple's FY\,2024 Form 10-K showing one-time tax anomaly disclosures: page~24 (top) and page~39 (bottom).}
...
\caption{Training loss curves before accounting for tax anomalies.}
...
\caption{Training loss curves after accounting for tax anomalies.}
...
\caption{Example output from Step~4: supplementary table extractions from the annual report.}
...
\caption{Hallucination analysis across 101 LLM extraction runs: GM (top), LVMH (middle), and Tencent (bottom).}
...
\caption{Hallucination analysis (continued): Alibaba (top), JPMorgan Chase (middle), and ExxonMobil (bottom).}
...
\caption{Hallucination analysis (continued): Volkswagen (top), Microsoft (middle), and Google (bottom).}
