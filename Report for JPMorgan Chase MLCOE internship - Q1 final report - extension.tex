...
\pagestyle{fancy}
\fancyhf{}
\rhead{\color{jpblue}\textbf{JP Morgan --- Q1 Final Report}}
...
\begin{titlepage}
  ...
  {\large JP Morgan --- MLCOE Internship Project (Question 1)\par}
  \vspace{0.5cm}
  {\normalsize Jaebum (Albert) Chung\par}
  \vspace{0.3cm}
  {\normalsize February 26, 2026\par}
  \vspace{3cm}
  \begin{abstract}
    \noindent This report implements a structural financial statement forecasting model
    based on the no-plug framework of Pareja (2009), where the balance sheet is derived
...
equal and opposite entries across accounts, so that the fundamental
...
expenditure)---net cash impact is zero. On the Balance Sheet,
...
These two examples illustrate the principle that underpins the entire
forecasting model in this report. In our implementation, the Statement
of Cash Flows is realized as a \emph{Cash Budget} decomposed into
five net liquidity balances: operating, capital expense, financing,
owner transactions, and discretionary. Every revenue event, expense,
and cash movement is explicitly routed through the appropriate Cash
Budget module, and every accrual is tracked on the Balance Sheet.
Because no ``plug'' variable forces the equation to balance, the
accounting identity
$\text{Assets} = \text{Liabilities} + \text{Equity}$ must
\emph{emerge} from the correctness of the model's logic. If a single
cash flow is misrouted or an accrual is omitted, the identity breaks
and the error is immediately detectable---this is the core validation
mechanism described in Section~\ref{sec:testing}.
...
company's liquidity strategy \hyperlink{ref:almeida14}{[Almeida(14)]}. This
...
Revenue as the single primary driver. All other elements---cost of
goods sold, inventory, receivables, payables, and liquidity---are
derived from Sales through trained policy ratios, while Purchases
is computed endogenously from the inventory balance identity rather
than forecast independently. By anchoring the forecast to this core
operational activity, the model can accurately predict the evolution
of the balance sheet and income statement while maintaining the
necessary accounting linkages.
...
\subsection{1. The Balance Sheet Elements}
...
\subsection{2. Mathematical Equations Governing the Balance Sheet Elements}
...
\paragraph{Non-Current Assets (NCA)}
...
\subsubsection{Liquidity Budget}
...
\paragraph{Logit-Linear Trend} We calculate the target short-term debt
...
\paragraph{Effective ST Debt Calculation} The actual target volume is
...
\subsection{3. Time Series Representation of the Balance Sheet}
...
\subsubsection{1. Domain Constraints on Trainable Variables}
...
\subsubsection{2. Logical Constraints on Equations}
...
is found in \texttt{sec-edgar-download-extractor.py}.
...
\subsection{4. Training the Model}
...
running \texttt{run\_trainable\allowbreak{}\_model\allowbreak{}\_forecast\allowbreak{}\_simple\allowbreak{}\_policies.py}:
...
\subsection{5. Applying Machine Learning}
...
\subsection{6. Projected Balance Sheet, Income Statement, and Cash Budget}
...
\subsubsection{Module 1: Operating Activities}

\paragraph{Cash Inflows}

Inflows from sales

\emph{Total inflows = Inflows from sales}

\paragraph{Cash Outflows}
...
\paragraph{Operating Net Cash Balance}

= Total inflows -- Total outflows

= Sales -- COGS -- OpEx -- Income Tax

\subsubsection{Module 2: Investing Activities}

\paragraph{Cash Inflow}

\emph{Total inflows = 0}

\paragraph{Cash Outflow}

Investment in fixed assets = CapEx

\emph{Total outflows = CapEx}

\paragraph{Investing Net Cash Balance}

= Inflow -- Outflow

= -CapEx

\subsubsection{Module 3: External Financing}
...
\paragraph{Financing Net Cash Balance}
...
\subsubsection{Module 4: Transactions with Owners}
...
\paragraph{Owners' Transaction Net Cash Balance}

= Inflows -- Outflows

= Equity financing -- Dividends from last year -- Stock buyback

\subsubsection{Module 5: Discretionary Transactions}
...
\paragraph{Discretionary Transaction Net Cash Balance}
...
where $\beta_0, \beta_1$ have Normal variational posteriors with
...
\textbf{Response:} Thank you. We have added a new section \ul{Parameter
...
\item \textbf{Automatic differentiation.}
  ...
  every parameter---impractical given the interdependencies of the
