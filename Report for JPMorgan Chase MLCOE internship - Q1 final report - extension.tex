...
\textbf{Final Report for JPMorgan Chase MLCOE internship -- Question 1}
...
\item
  Mathematical Equations Governing the Balance Sheet Elements
\item
  Time Series Representation of the Balance Sheet
\item
  Training the Model
\item
  Applying Machine Learning
\item
  Projected Balance Sheet, Income Statement, and Cash Budget
\end{enumerate}
\end{quote}
...
\item
  LLM Selection and Infrastructure Setup
\item
  LLM-Only Balance Sheet Forecasting
\item
  Combining LLM with the Balance Sheet Forecasting Model (Tax Anomalies)
\item
  Strategic Recommendations for CEO/CFO
\item[e, f, g)] Financial Ratios and Automated Extraction Pipeline
\item[h, i)] Applicability to Other Annual Reports
\end{enumerate}
\end{quote}
\end{quote}
...
the double-entry accounting system. As reviewed in Pareja et al. (2007)
and Pareja (2009), a significant portion of existing literature relies
...
often methodologically flawed. Because these \textquotesingle black
box\textquotesingle{} models treat variables as probabilistic time
...
\ul{Proposed Methodology: A Structural "No-Plug" Approach}

To address these limitations, this report implements a structural
forecasting model based on the framework proposed by Pareja (2009). The
core distinction of this method is the rejection of circular references
and balancing plugs. Instead, the model relies on a fully integrated
system where the Balance Sheet is derived endogenously from the Cash
Budget and Income Statement.

In this approach, every line item is driven by specific causal factors
(e.g., Receivables driven by Sales, Payables driven by Operating
Expense) and the calculation of debt, and the ending cash balance is a
calculated result of these flows rather than a residual plug.

In Pareja (2009)'s framework, all the policy variables are known, and
the models are used to forecast the balance sheet based on predicted
sales and purchases accounting for pricing and volume relationship and
inflation. In our case, we do not know the policy variables, but know a
company's past financial statements. Using simple linear equations and
machine learning algorithms, we train the policy variables that will
satisfy the historical financial data. We initially assume that the
variables remain constant within our training data, and in later
sections apply machine learning methods to account for probability
distribution of certain variables to provide richer predictions.

We have further refined the Pareja (2009) framework to better reflect
the treasury strategies of modern multinational corporations like Apple
or General Motors. Where the original model might treat Marketable
Securities solely as a reservoir for excess cash, our implementation
models these securities as a distinct portion of the
company\textquotesingle s liquidity strategy \hyperlink{ref:almeida14}{[Almeida(14)]}. This
adjustment allows for a more realistic depiction of how large firms
manage capital allocation between operational cash and interest-bearing
assets.
...
Once the relationships and policy variables connecting the financial
statement elements are established, the methodology utilizes Sales
Revenue and Purchases as the central drivers. By anchoring the forecast
to these core operational activities, the model can accurately predict
the evolution of the balance sheet and income statement while
maintaining the necessary accounting linkages.

\ul{Testing and Validation Plan}

The validity of this implementation is verified through a rigorous
"Balance Check." Because the model eschews plugs, the fundamental
accounting identity (\emph{Assets = Liabilities + Equity}) serves as a
strict test of correctness rather than a guaranteed outcome.

Our testing plan involves running the forecast over a multi-year horizon
and calculating the delta between Total Assets and Total Claims at every
time step. In this "no-plug" environment, a delta of zero confirms that
all cash inflows and outflows have been correctly captured and routed
through the Cash Budget. Any deviation from zero indicates a specific
error in the logic (e.g., a "leak" in the system where an expense was
recorded but cash was not deducted), allowing us to pinpoint and resolve
structural flaws that traditional models would mask.

The following sections (\textbf{1. The Balance Sheet Elements},
\textbf{2. Mathematical Equations Governing the Balance Sheet Elements,}
and \textbf{3. Time Series Representation of the Balance Sheet)} are
answers to the questions 1 and 2, submitted codes are answers to
questions 3 and 4, and the remaining sections (\textbf{4. Training the
Model} and \textbf{5. Applying Machine Learning}) provide answers to
questions 5, 6, 7, 8. For Part 2, the answers to the questions a)
\textasciitilde{} i) are directly mapped to its headings.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{The Balance Sheet Elements}
\end{enumerate}
...
below, using GM's balance sheet as example:
...
\begin{itemize}
\item
  ST debt, current accrued expenses, other current liabilities, and
  current portion of the long-term debt and capital lease obligations
  principals
\item
  \textbf{STLoan\textsubscript{t}} = ST debt\textsubscript{t} + current
  accrued expenses\textsubscript{t} + other current
  liabilities\textsubscript{t}
\item
  \textbf{CLiab\textsubscript{t}} = \textbf{STLoan\textsubscript{t}} +
  Total LT liability\textsubscript{t} / \textbf{AvgM} =
  \textbf{STLoan\textsubscript{t}} + \textbf{NLiab\textsubscript{t}} /
  (\textbf{AvgM} -- 1)
\end{itemize}
...
We further upgrade the models to better represent Apple's historical
data, namely the effective ST debt in Current liabilities:
...
\item
  ...
  \begin{itemize}
  \item
    Modeled as a logit-linear policy ratio of sales:
    EffSTDebt\textsubscript{t} = Sales\textsubscript{t} *
    \%STDebt\textsubscript{t}
  \end{itemize}
\item
  ...
  \end{itemize}
\item
  Total Current Liabilities Calculation: CLiab\textsubscript{t} =
...
\setcounter{enumi}{1}
\item
  \textbf{Mathematical Equations Governing the Balance Sheet Elements}
\end{enumerate}
...
modeled independently based on historical data.

We begin by modeling the asset elements.
...
terms of our modeled variables. During model training, the optimizer
...
\setcounter{enumi}{1}
\item
  \emph{Cost of Goods Sold (COGS) and Inventory}
\end{enumerate}
...
and 1, we transform the historical cost ratio using the logit function
and model it as a linear function of time (t):
...
As done in \hyperlink{ref:pareja09}{Pareja(09)}, we set the inventory target as a percentage of
sales:

\begin{equation}\label{eq:inventory_target}
\text{Inv}_t = \text{\%Inv} \cdot \text{Sales}_t
\end{equation}

By substituting our known variables into \eqref{eq:inventory_rollforward}, we can solve for
Purchases\textsubscript{t}, which now acts as the necessary plug to
balance the inventory account:
...
We add \%Inventory (\%Inv), $\alpha$\textsubscript{CR} and $\beta$\textsubscript{CR}
...
\setcounter{enumi}{2}
\item
  \emph{Advance Payments for Purchases}
\end{enumerate}
...
percentage of sales. Within the liquidity, we assume that a portion of
it is invested in market securities (IMS):
...
We add \%TL and \%Cash to our variables to be trained.

Before we move onto liabilities, it is helpful to generate a Liquidity
Budget to model the flow of cash in and out of cash balance and
short-term investment into market securities. We have updated the name
from Cash Budget to Liquidity Budget to reflect our use of total
liquidity to represent both cash and market securities investment.
...
LTLoan\textsubscript{t} in \ul{2.3 Liabilities}.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\setcounter{enumi}{1}
\item
  \emph{External Investment NLB}
\end{enumerate}

We redeem external investments from last year with a fixed percentage of
return (\%MSReturn), and make new investments this year:
...
\setcounter{enumi}{2}
\item
  \emph{Transaction with owners NLB}
\end{enumerate}
...
\text{Target Dividends}_{t-1} = \text{\%Payout ratio} \cdot \text{Net income}_{t-1}
\end{equation}

Rather than calculating dividends strictly as a fixed percentage of
current net income, we apply the Lintner Model to account for dividend
smoothing. Corporate management typically prefers stable dividend
payouts to avoid signaling financial distress to investors. Therefore,
the modeled dividend is represented as a weighted average between a
target payout (driven by a baseline \%Payout\_Ratio) and the actual
dividends paid in the prior period, regulated by an optimized
Adjustment\_Speed parameter. So, the actual dividends equation becomes:

\begin{equation}\label{eq:lintner_dividends}
\text{Dividends}_{t-1} = \text{Adj.\ Speed} \cdot \text{Target Dividends}_{t-1} + (1 - \text{Adj.\ Speed}) \cdot \text{Dividends}_{t-2}
\end{equation}

\begin{equation}\label{eq:stock_buybacks}
\text{Stock buybacks}_t = \text{\%Buyback ratio} \cdot \text{Depreciation}_t + \text{Any excess cash after meeting liquidity target}
\end{equation}
...
will define the equations for Net income\textsubscript{t} in \ul{2.4.
Income Statement}.
...
We obtain the final net liquidity balance\textsubscript{t} by summing
all the different activities' NLB\textsubscript{t}:
...
This is how we obtain the net liquidity balance at time t without plugs
or circularity. The unknown variables in the equation can be trained
because we have the target liquidity balance given by \eqref{eq:total_liquidity}:

$\text{TL}_t = \text{\%TL} \cdot \text{Sales}_t$ from \eqref{eq:total_liquidity}
...
We adjust how we model the current liabilities to better fit Apple's
historical data.

\subsubsection{Modeling Effective Short-Term
Debt}\label{modeling-effective-short-term-debt}
...
The forecasted Current and non-current liabilities thus fall to zero:

\includegraphics[width=\textwidth]{report_latex_media/media/image1.png}

\ul{Defining the Metric}
...
\ul{The Solution \& Implementation}
...
deficit-driven reaction. It is projected as a logit-linear ratio of
sales, mirroring the methodology used for the cost ratio and target
liquidity.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Logit-Linear Trend:} We calculate the target short-term debt
  as a percentage of sales using a sigmoid function over the time index
  t. This utilizes the learned parameters $\alpha$\textsubscript{ST\_Debt} and
  $\beta$\textsubscript{ST\_Debt}:
\end{enumerate}

\begin{equation}\label{eq:pct_stdebt_sigmoid}
\text{\%STDebt}_t = \text{sigmoid}(\alpha_{\text{ST\_Debt}} + t \cdot \beta_{\text{ST\_Debt}})
\end{equation}
...
\setcounter{enumi}{1}
\item
  \textbf{Effective ST Debt Calculation:} The actual target volume is
  then derived directly from the forecasted sales for that period:
\end{enumerate}

\begin{equation}\label{eq:eff_st_debt_calc}
\text{Effective ST Debt}_t = \text{Sales}_t \cdot \text{\%STDebt}_t
\end{equation}
...
\setcounter{enumi}{2}
\item
  \textbf{Liquidity Cascade:} Because short-term debt is now an
...
Next year's dividends were already modeled in \eqref{eq:target_dividends}

$\text{Dividends}_t = \text{\%PR} \cdot \text{NI}_t$ from \eqref{eq:target_dividends}
...
\setcounter{enumi}{2}
\item
  \textbf{Time Series Representation of the Balance Sheet}
\end{enumerate}
...
\item
  \%AdvPS : Advance payments from customers as a percentage of next
  year's sales
\item
  \%AdvPP : Advance payments to suppliers as a percentage of next year's
  purchases
\item
...
\item
  $\alpha$\textsubscript{CR} and $\beta$\textsubscript{CR} : Variables that make up
  the Cost Ratio via logit model
  ...
  \end{itemize}
\item
  \%TL : Total liquidity as a percentage of this year's sales
\item
  \%Cash : Cash as a percentage of total liquidity
\item
  \%IT : Income tax as a percentage of net income this year
\item
  \%OR : Variable portion of OpEx that increases in proportion to this
  year's sales
\item
  OB\textsubscript{T\_start} : Baseline OpEx that increases with this
  year's average inflation
\item
  $\alpha$\textsubscript{ST\_Debt} and $\beta$\textsubscript{ST\_Debt} : parameters
  for sigmoid function for calculating Effective ST debt
\item
  \%AvgSTInt : Average short-term interest on the effective short-term
  loan
\item
...
\item
  \%MSReturn : Percentage return on the investment in market securities
\item
  \%EF : Equity financing as a percentage of total excess cash needed
  after accounting for long-term investments and long-term obligations
\item
  \%PR : Dividends payout ratio as a percentage of net income this year,
  paid next year
\item
  Adj. speed : Speed at which to adjust the dividends amount paid next
  year
\item
  \%BB : Stock buyback as a percentage of asset depreciation this year

  \begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \item
    \ul{Parameter Constraints and Bounds}
  \end{enumerate}
\end{itemize}
...
\textbf{1. Domain Constraints on Trainable Variables}

\begin{itemize}
\item
  \textbf{Strict Positivity (\$x \textgreater{} 0\$):} Variables that
  cannot logically be negative, such as the Depreciation Rate (\%Depr),
  Asset Growth (\%AG), and Interest Rates (\%AvgSTInt, \%AvgLTInt), are
  constrained using a Softplus activation function. This prevents the
  optimizer from testing negative rates during gradient descent.
\item
  \textbf{Percentage Bounds (\$0 \textless{} x \textless{} 1\$):}
  Variables representing fractions of a whole, such as the Income Tax
  Percentage (\%IT), Dividend Payout Ratio (\%PR), and the Lintner
  Dividend Adjustment Speed (\$\textbackslash alpha\$), are constrained
  using a Sigmoid function.
\item
  \textbf{Maturity Boundary (\$AvgM \textgreater{} 1.001\$):} Because
  the calculation for the current portion of long-term debt divides by
  \$(AvgM - 1)\$, the average maturity must strictly exceed 1 year to
...
\begin{itemize}
\item
  \textbf{Trended Ratios:} Time-varying policies, such as the Cost Ratio
  (CR) and Effective Short-Term Debt percentage, are modeled as
  logit-linear trends. By wrapping the linear trend
  \$(\textbackslash alpha + \textbackslash beta t)\$ in a sigmoid
  function, the forecasted ratios are strictly bounded between 0 and 1,
  even if extrapolated decades into the future.
\end{itemize}

\textbf{2. Logical Constraints on Equations}

\begin{itemize}
\item
  \textbf{Non-Negative Borrowing:} A company cannot issue "negative"
  debt to reduce cash. In the Liquidity Budget equations, the issuance
  of new short-term and long-term loans is constrained using a maximum
  function: \$New\textbackslash\_Loan\_t = \textbackslash max(0,
  Liquidity\textbackslash\_Deficit\_t)\$. If there is a cash surplus,
...
\item
  ...
  equation (\$Assets = Liabilities + Equity\$) is strictly evaluated at
  every time step \$t\$. Rather than enforcing this via a loss function
  penalty (which would allow minor deviations), it is enforced
  structurally by the double-entry routing of the Cash Budget. The
  simulation calculates a delta to verify that the mismatch is exactly
  zero.

  \begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \item
    \ul{Variables from External Environment}
  \end{enumerate}
\end{itemize}
...
\item
  ...
  \begin{itemize}
  \item
    CLiab\textsubscript{t}
  \end{itemize}
\end{itemize}

\begin{quote}
= STLoan\textsubscript{t} + NLiab\textsubscript{t} / (AvgM -- 1)
...
\begin{itemize}
\item
  STLoan\textsubscript{t} = max(0, STCashDeficit\textsubscript{t})
\item
  STCashDeficit\textsubscript{t} = TL\textsubscript{t} --
  TL\textsubscript{t-1} -- Operating NLB\textsubscript{t} +
  STLoan\textsubscript{t-1} * (1 + \%AvgSTInt) -- \%MSReturn *
  IMS\textsubscript{t-1}
\item
  STLoan\textsubscript{t-1} = CLiab\textsubscript{t-1} -
  NLiab\textsubscript{t-1} / (AvgM -- 1)
\item
...
\item
  ...
  \begin{itemize}
  \item
    NLiab\textsubscript{t}
  \end{itemize}
\end{itemize}

\begin{quote}
= (LTLoan\textsubscript{t} + NLiab\textsubscript{t-1}) * (AvgM --
1)/AvgM
...
\begin{itemize}
\item
  LTLoan\textsubscript{t} = (max(0, STCashDeficit\textsubscript{t} --
  STLoan\textsubscript{t} -- CapEx NLB\textsubscript{t} +
  NLiab\textsubscript{t} / (AvgM -- 1) + AvgLTInt\textsubscript{t} +
  Dividends\textsubscript{t-1} + Stock buybacks\textsubscript{t}) * (1 -
  \%EF)
\item
  CapEx NLB\textsubscript{t} = NCA\textsubscript{t-1} * \%Depr + \%AG *
  Sales\textsubscript{t}
\item
  Dividends\textsubscript{t-1} = \%PR * NI\textsubscript{t-1}
\item
...
AvgSTInt\textsubscript{t-1} -- AvgLTInt\textsubscript{t-1} + \%MSReturn
* (1 - \%Cash) * TL\textsubscript{t-1})
\end{quote}

\begin{itemize}
\item
  TL\textsubscript{t-1} = Total liquidity last year.
\item
...
\item
  AvgSTInt\textsubscript{t} = \%AvgSTInt * (CLiab\textsubscript{t-t} --
  NLiab\textsubscript{t-t} / (AvgM -- 1))
\item
  AvgLTInt\textsubscript{t} = \%AvgLTInt * (NLiab\textsubscript{t-1} /
  (1 -- 1/AvgM))
\item
  Stock buybacks\textsubscript{t} = \%BB * Depreciation\textsubscript{t}
  + excess cash after meeting liquidity target
\end{itemize}
...
\begin{quote}
= SE\textsubscript{t-1} + LTLoan\textsubscript{t} / (1 - \%EF) * \%EF +
NI\textsubscript{t} -- \%PR * NI\textsubscript{t-1} -- \%BB * \%Depr *
NCA\textsubscript{t-1} -- excess cash after meeting liquidity target
\end{quote}
...
\textbf{simple\_financial\_model.py} using data from Apple's financial
statements. The financial statements were first obtained from Yahoo
finance as suggested by Question 4 (we wrote the script in
\textbf{yahoofinance.py}), but we could only access past 4 years of
...
is found in \textbf{sec-edgar-download-extractor.py.}

The following is the output of the code when run with: python
.\textbackslash simple\_financial\_mode.py :

\includegraphics[width=4.67615in,height=0.9667in]{report_latex_media/media/image2.png}
...
Out of the variables to be trained, there are quite a few that are
easily trained independently from other variables by looking at balance
sheets and income statements. We first note the primary driver for our
model:

\begin{itemize}
\item
  Sales\textsubscript{t} : Total revenue in balance sheet
\end{itemize}

We can train the rest of the variables like the following:

\begin{itemize}
\item
  \%AG, \%AM : Learn from change in all non-current assets in balance
  sheet
\item
  \%Depr : Learn from depreciation in income statement and non-current
  assets in balance sheet
\item
  $\alpha$\textsubscript{CR} , $\beta$\textsubscript{CR} : Learn from COGS and Sales
  data
\item
  \%AR : Learn from Sales and account receivables data in balance sheet
\item
  \%AP : Learn from Purchases and account payables data in balance sheet
\item
  \%AdvPS : Learn from current deferred revenue data and current year's
  Sales in balance sheet
\item
  \%AdvPP : Learn from Purchases and current other assets in balance
  sheet
\item
  \%Inv : Learn from inventory data in balance sheet
\item
  \%Cash : Learn from short-term investments in balance sheet
\item
  \%TL : Learn from revenue and Cash Equivalents And Short Term
  Investments in balance sheet
\item
  \%IT : Learn from net income and tax provision in income statement
\item
  \%OR, OB\textsubscript{T\_start} : Learn from inflation data and
  operating expense in income statement
\item
  \%PR, Adj. Speed : Learn from common stock dividends in Cash Flow and
  net income in income statement
\item
  \%BB : Learn from Repurchase Of Capital Stock and depreciation in Cash
  Flow
\end{itemize}

Each of the variables above can be related to an element from Cash Flow,
Balance Sheet, or Income Statement, and can be trained with a simple
mean-square-error minimization for each.

The following variables (i.e. structural parameters) can be trained
together using the above trained variables:
...
Using net income, interest payments, return from investment in market
securities, stockholders equity and current and non-current
liabilities data, we can train AvgM, \%AvgSTInt, \%AvgLTInt, \%MSReturn,
and \%EF concurrently via simple gradient while minimizing mean-square
error between predicted and actual net income, stockholders equity,
current liabilities, and non-current liabilities.

The code has been implemented in trainable\_financial\_model.py. For
...
parameters from Apple's 2018\textasciitilde2024 data, predict 2025 data,
and compare it with the actual data. The following is the output after
running \textbf{trainable\_financial\_model.py}:

\includegraphics[width=\textwidth]{report_latex_media/media/image3.png}

It shows a modest prediction ability, which is unsurprising because we
have lumped various balance sheet elements together to form several
elements to be more consistent with models described in \hyperlink{ref:pareja07}{Pareja(07)} and
\hyperlink{ref:pareja09}{Pareja(09)}, as we have explained in \textbf{1. The Balance Sheet
Elements}. Also, there is a limitation of assuming a liquidity balance
(cash + investment in market securities) that varies linearly with
sales, especially for a cash-rich company like Apple whose cash balance
and stock buybacks are governed by the recent buyback programs in order
to reduce its cash balance instead of being proportional to asset
depreciation, which is done here as done by \hyperlink{ref:pareja09}{Pareja(09)}.

Our model can also forecast earnings because net income is automatically
calculated when predicting the elements on the balance sheet, and is
outputted as ``net\_income'' as shown above.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\setcounter{enumi}{4}
\item
  \textbf{Applying Machine Learning}
\end{enumerate}
...
Most of the code from trainable\_financial\_model.py remains unchanged
except how we model the OpEx during training. The following updates are
made:
...
{\sloppy We implemented the updated Bayesian model in \textbf{trainable\_financial\_model\_taxanomaly\_effstdebt\_bayesian\_vi\_refactored.py}
with the include\_tax\_anomalies = False, and the output is reproduced
below:\par}
...
-----|----------|----------|----------
2025 | 1.09e+11 | 1.04e+11 | 1.14e+11
2026 | 1.20e+11 | 1.15e+11 | 1.25e+11
2027 | 1.30e+11 | 1.25e+11 | 1.35e+11
2028 | 1.41e+11 | 1.35e+11 | 1.47e+11
2029 | 1.52e+11 | 1.46e+11 | 1.58e+11
2030 | 1.63e+11 | 1.57e+11 | 1.70e+11
2031 | 1.75e+11 | 1.68e+11 | 1.81e+11
2032 | 1.87e+11 | 1.80e+11 | 1.95e+11
2033 | 2.00e+11 | 1.92e+11 | 2.07e+11
2034 | 2.12e+11 | 2.04e+11 | 2.20e+11
\end{lstlisting}

\includegraphics[width=\textwidth]{report_latex_media/media/image5.png}
\includegraphics[width=\textwidth]{report_latex_media/media/image6.png}
\includegraphics[width=\textwidth]{report_latex_media/media/image7.png}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\setcounter{enumi}{5}
\item
  \textbf{Projected Balance Sheet, Income Statement, and Cash Budget}
\end{enumerate}

We describe how we calculate the cash budget and income statement which
are related to the balance sheet.

We can start with the revenue on the income statement which appears at
the top.

Revenue is equivalent to Sales\_t.

It is followed by operating costs, which include:

\begin{itemize}
\item
  Cost of goods sold (COGS)
\item
  Operational expenses (OpEx such as administrative and sales)
\item
  Depreciation and amortization
\end{itemize}

EBIT (earnings before interests and taxes) is calculated by

EBIT = Revenue -- COGS -- OpEx -- Depreciation

Then, we account for interest payments and ST investment returns, which
are used to calculate EBT (earnings before taxes):

EBT = EBIT -- interest payments + ST returns

Then, we assess the income tax by multiplying EBT by the effective tax
rate:

Income Taxes = EBT * \%EffTax

Then, net income is found by subtracting the income taxes from EBT:

Net Income = EBT -- Income Taxes

From the net income, we allocate a percentage of it for dividends to be
paid out next year, and the rest goes to cumulated retained earnings
(CRE) which is part of equity:

Next year dividends = Net Income * \%Payout

CRE\_t = CRE\_t-1 + Net Income

Cash Budget has 5 modules: Operating Activities, Investing Activities,
External Financing, Transactions with Owners, and Discretionary
Transactions.

\textbf{Module 1: Operating Activities}

\ul{Cash Inflows}

Inflows from sales

\emph{Total inflows = Inflows from sales}

\ul{Cash Outflows}

Payments for purchases = COGS

Operational Expenses = OpEx

Income Tax

\emph{Total outflows = COGS + OpEx + Income Tax}

\ul{Operating Net Cash Balance}

= Total inflows -- Total outflows

= Sales -- COGS -- OpEx -- Income Tax

\textbf{Module 2: Investing Activities}

\ul{Cash Inflow}

\emph{Total inflows = 0}

\ul{Cash Outflow}

Investment in fixed assets = CapEx

\emph{Total outflows = CapEx}

\ul{Investing Net Cash Balance}

= Inflow -- Outflow

= -CapEx

\textbf{Module 3: External Financing}
...
\emph{Total inflows = ST loan + LT loan}
...
\emph{Total outflows = ST principal + ST interest + LT principal + LT
interest}

\ul{Financing Net Cash Balance}

= Inflow -- Outflow

= ST loan + LT loan -- ST principal -- LT principal -- ST interest -- LT
interest

\textbf{Module 4: Transactions with Owners}

\ul{Cash Inflows}

Invested equity = Equity financing

\emph{Total Inflows = Invested equity}
...
\emph{Total Outflows = Dividends from last year + Stock buyback}

\ul{Owners' Transaction Net Cash Balance}

= Inflows -- Outflows

= Equity financing -- Dividends from last year -- Stock buyback

\textbf{Module 5: Discretionary Transactions}

\ul{Cash Inflows}

Return from ST investments

\emph{Total inflows = Return from ST investments}

\ul{Cash Outflows}

No outflow because our model's policy allocates a fraction of total
liquidity to ST investments instead of allocating excess cash to it

\ul{Discretionary Transaction Net Cash Balance}
...
\ul{Response:} Thank you for catching this mistake. This was a typo in
...
\ul{Response:} FIFO (First-In, First-Out): Under FIFO, the oldest
...
Statement, and Cash Budget in \textbf{6. Projected Balance Sheet, Income
...
\ul{Response:} We updated our model such that the advance payments for
purchases (AdvPP) are proportional to this year's purchases instead of
next year, with the justification:

\begin{itemize}
\item
  From 2.1.3. Advance Payments for Purchases : ``Advance payments to
...
\begin{itemize}
\item
  From 2.3.2. Advance Payments for Future Sales: ``Advance payments
  received from customers for future sales (\$AdvPC\$) are modeled as a
  direct proportion of the current year\textquotesingle s total sales.
...
\ul{Response:} We updated our model to handle the case where the asset
...
\ul{Response:} We have plotted the forecasted debt levels (see above).
The outcome is sensible and mathematically stable. We verified that the
model does not fall into an explosive \textquotesingle debt
spiral.\textquotesingle{} The financing dynamics converge realistically.
...
\item
  ...
  Apple\textquotesingle s real-world strategy of utilizing revolving
...
\ul{Response:} We can redefine our training variables from arbitrary
...
\ul{Response:} Thank you. We have added a new section \ul{Parameter
...
\ul{Response:} We now include the 10-year forecasted Balance Sheet,
Income Statement, and Cash Budget in \textbf{6. Projected Balance Sheet,
Income Statement, and Cash Budget.}

(10) Remember to write test cases.

Response: Thank you for the reminder. We wrote test cases for all the
scripts appearing in this report:

simple\_financial\_model.py

trainable\_financial\_model.py

llm-only-balance-sheet-forecast.py

llm\_extract\_tax\_anomalies\_from\_pdf.py

send\_reasoning\_prompt.py

\ul{llm\_extract\_financial\_statement\_from\_pdf.py}

llm\_extract\_json\_from\_financial\_statements.py

The test scripts have `test\_' in front of each name.
...
\def\labelenumi{\alph{enumi})}
\item
  \textbf{LLM Selection and Infrastructure Setup}
\end{enumerate}

My favorite LLM is DeepSeek V3.2 for reasoning which has 128k context
window and gpt-5-nano for non-reasoning task which has 400k context
window. From my daily usage, DeepSeekV3.2 follows instructions well with
less hallucinations than other models from competitors. Gpt-5-nano, from
my experience, is an inexpensive model that performs well for pretty
complicated tasks but hallucinates more. I use Microsoft's Azure Foundry
...
variable in a .env file, with its content being:

{\small AZURE\_DEEPSEEK\_ENDPOINT=\url{https://dru54knwxa7sb4rlwrgws3kaze0jdvtj.lambda-url.us-west-1.on.aws/}}
...
\setcounter{enumi}{1}
\item
  \textbf{LLM-Only Balance Sheet Forecasting}
\end{enumerate}

We can use the same input data from part 1 and feed it to DeepSeekV3.2
to forecast the balance sheet, as done in
llm-only-balance-sheet-forecast.py.
...
The returned fields are further processed within the script
\textbf{llm-only-balance-sheet-forecast.py} to ensure the balance sheet
...
\setcounter{enumi}{2}
\item
  \textbf{Combining LLM with the Balance Sheet Forecasting Model (Tax
  Anomalies)}
\end{enumerate}
...
(market securities return and interest payments are uncertain due to
incomplete data, but their combined amount is order of magnitude smaller
...
\noindent\begin{minipage}{0.48\textwidth}\centering\includegraphics[width=\linewidth]{report_latex_media/media/image16.png}\end{minipage}\hfill\begin{minipage}{0.48\textwidth}\centering\includegraphics[width=\linewidth]{report_latex_media/media/image17.png}\end{minipage}
...
Script: \textbf{llm\_extract\_tax\_anomalies\_from\_pdf.py}

\begin{lstlisting}[style=python]
DEFAULT_SELECTION_QUERY = (
    "Item 7 Management's Discussion and Analysis (MD&A), "
    "and Item 8 Financial "
    'Statements and Supplementary Data sections specifically '
    'covering "Income Taxes" '
...
DEFAULT_EXTRACTION_PROMPT = (
    ...
    "provided excerpts "
    "from a company's Form 10-K (MD&A and Financial Footnotes) and "
    "extract data regarding "
    "one-time tax anomalies and future tax contingencies.\n\n"
    "Carefully evaluate the text for the following:\n\n"
    "Current Year Anomalies: Identify any massive, non-recurring, "
    "discrete tax charges "
    "or benefits that heavily skewed the current year's net income "
    "(e.g., finalized state "
    "aid decisions, sudden impacts from new tax legislation).\n\n"
    "Future Contingencies: Identify any quantified maximum tax "
    "exposures, unreserved tax "
    "liabilities, or significant unrecognized tax benefits (UTBs) "
    "that management indicates "
    "could be resolved or assessed in future years (e.g., tax funds "
    "held in escrow pending "
    "appeal, estimated UTB decreases in the next 12 months).\n\n"
    "You must respond ONLY with a valid JSON object using the exact "
    "schema below. Do not "
    "include any markdown formatting, preamble, or postscript. If a "
    "specific data point is "
    "not explicitly mentioned or cannot be reliably quantified from "
    "the text, output null "
    "for that field.\n\n"
    "JSON Schema:\n"
    "{{\n"
    ...
    ' "tax_contingency_note": "<string explaining the future reserve '
    'or null>"\n'
    "}}\n\n"
    "Pages:\n{pages}"
)
...
{
    "query": "Item 7 Management's Discussion and Analysis (MD&A), and Item 8 Financial Statements and Supplementary Data sections specifically covering \"Income Taxes\" and \"Commitments and Contingencies\".",
    "selected_pages": [
    ...
    "extraction": {
        ...
        "tax_contingency_amount": 13,
        "tax_contingency_note": "Gross unrecognized tax benefits could decrease between approximately $5 billion and $13 billion in the next 12 months, primarily related to intercompany transfer pricing and deemed repatriation tax."
    }
}
\end{lstlisting}
...
run the script
trainable\_financial\_model\_taxanomaly\_effstdebt\_bayesian\-\_vi\_refactored.py
with include\_tax\_anomalies=True.
...
Before accounting for tax anomalies. The predicted net income is 1.09e11
dollars, which is off by 2.68\%, or 3 billion dollars, from the actual
value of 1.12e11 dollars.
...
\noindent\begin{minipage}{0.48\textwidth}\centering\includegraphics[width=\linewidth]{report_latex_media/media/image24.png}\end{minipage}\hfill\begin{minipage}{0.48\textwidth}\centering\includegraphics[width=\linewidth]{report_latex_media/media/image25.png}\end{minipage}

After accounting for tax anomalies. The predicted net income in 2025 is
1.11e11 dollars, which is only off by 0.89\%, or 1 billion dollars, from
the actual value of 1.12e11 dollars! The income tax MSE loss during
training also drops by an order of magnitude.

\begin{enumerate}
\def\labelenumi{\alph{enumi})}
\setcounter{enumi}{3}
\item
  \textbf{Strategic Recommendations for CEO/CFO}
\end{enumerate}
...
You are an elite Corporate Finance Advisor and Strategic Capital Expert
at a top-tier investment bank. You specialize in advising the CEOs and
CFOs of mature, cash-generative mega-cap technology firms on capital
structure optimization, liquidity management, and capital allocation.
...
ML Forecast: A multi-year financial forecast (Balance Sheet, Income
Statement, and Cash Budget) generated by an advanced Monte Carlo machine
learning ensemble.

Your task is to analyze the historical data against the forecast to
identify the three most critical financial leverage points (risks or
opportunities) over the forecasted period.

STRICT GUARDRAILS (CRITICAL):

NO OPERATIONAL ADVICE: You are strictly forbidden from recommending
operational, marketing, or business strategy changes. Do not suggest
"launching new products," "increasing sales efforts," or "improving
supply chain efficiency."

STRICTLY FINANCIAL DOMAIN: Your recommendations must be confined purely
to Corporate Finance: Capital Structure (debt/equity mix, WACC),
Liquidity \& Treasury (working capital, commercial paper), and Capital
Allocation (dividends, share repurchases, CapEx).

MANDATORY EVIDENCE: You cannot make a claim without citing specific
numbers from the provided tables.

Analytical Heuristics (The Mega-Cap Tech Lens \& Trend Analysis):

Historical Benchmarking (Crucial): Always contextualize the forecast by
comparing it to the historical baseline. Identify structural breaks
(where the forecast deviates significantly from historical trends) or
trend accelerations.

Strategic Leverage vs. Distress: Do not penalize shrinking Accounting
Equity. If Equity is declining due to aggressive Stock Buybacks and
Dividends, treat this as highly efficient Capital Return and WACC
optimization---NOT financial distress---provided Net Income and Free
Cash Flow remain historically strong.

Net Debt Focus: Evaluate liquidity by looking at Cash + Marketable
Securities against Liabilities. Mega-cap tech companies often issue debt
simply to lower their cost of capital; evaluate if the forecasted debt
load is easily serviced by operating cash flows relative to historical
interest coverage.

Operating Leverage: Note if the forecast predicts Revenue and Net Income
growing at a faster or slower rate than historical Operating Expenses
(OpEx).

Output Format:

Provide exactly 3 strategic priorities. For each, use this exact
structure:

Observation: {[}State the specific quantitative trend, explicitly
contrasting the forecasted figures against the historical baseline. Cite
exact numbers and years{]}.

Implication: {[}Explain the financial/credit risk or the WACC
optimization opportunity this trend creates for the balance sheet{]}.

Recommendation: {[}State the precise corporate finance action the
CFO/CEO must execute to manage this specific trajectory{]}.

Data Input:

\emph{{[}Insert Historical Tables / JSON here{]}}

\emph{{[}Insert ML Forecast Tables / JSON here{]}}

``\,''\,''

The prompt with the context of the 10-year forecast is sent to our
reasoning model by calling send\_reasoning\_prompt.py

And here is the recommendation that our reasoning model generated:

\subsubsection{\texorpdfstring{\textbf{Analysis of Financial Leverage
Points}}{Analysis of Financial Leverage Points}}\label{analysis-of-financial-leverage-points}

\textbf{Observation:}~The forecast projects a significant and
accelerating decline in Stockholders\textquotesingle{} Equity, from
...
\textbf{Implication:}~This trajectory indicates an extremely aggressive
...
\textbf{Recommendation:}~The CFO must formally establish and communicate
...
\textbf{Observation:}~The forecast shows a dramatic increase
...
\textbf{Implication:}~The company is forecasted to become significantly
...
\textbf{Recommendation:}~Execute a~\textbf{Strategic Liability
...
company\textquotesingle s long-lived assets. Concurrently, establish a
minimum liquidity covenant (e.g., Cash + Securities $\geq$ 1.0x ST Debt) for
internal risk management.

\textbf{Observation:}~The forecast predicts a powerful~\textbf{positive
...
\textbf{Implication:}~This trajectory will generate super-linear growth
...
\begin{quote}
\textbf{Recommendation:}~Initiate a~\textbf{Dynamic Share Repurchase
...
\textbf{e, f, g) Financial Ratios and Automated Extraction Pipeline}
...
\textbf{List of fields to be extracted:}
...
\textbf{\ul{llm\_extract\_financial\_statement\_from\_pdf.py}}

\textbf{Primary financial statement extraction:}

\ul{Step 1: Page number identifying step}
...
\ul{Step 2: Table extraction step}
...
\textbf{Supplementary tables extraction:}

\ul{Step 3: Page number identifying step}
...
\ul{Step 4: Table extraction step}
...
\ul{Step 5: Extracting relevant fields as JSON for ratio calculation}
...
prompt = (
    ...
    "11) For taxes, it is the tax assessed on the income. If "
    "a tax belongs to the cost of revenue, it should be part "
    "of total_operating_cost.\n"
    "12) Generally, numbers in parentheses are negative.\n"
    "13) For taxes, interest_expenses, and "
    ...
    "negative.\n"
    "14) For short_term_market_securities, these are liquid, "
...
\ul{Before: LLM attempts to add the total debt by itself}
...
\ul{After: LLM lists out all the original elements without summation}
...
Run \textbf{llm\_extract\_json\_from\_financial\_statements.py} with
these arguments:

\begin{lstlisting}[style=console]
python llm_extract_json_from_financial_statements.py \
  --input-dir extracted_text \
  --num-extraction-runs 101 \
  --runs-output-dir deepseek_financial_statements_runs \
  --max-workers 9 \
  --plot-distributions
\end{lstlisting}

Then, run a simple script, \textbf{calculate\_hallucination\_rates.py},
to plot the extracted json data to visualize hallucination rates for
different financial fields from different companies:

\begin{lstlisting}[style=console]
python calculate_hallucination_rates.py \
  --run-values-file deepseek_financial_statements_runs/\
    runs_3_with_operating_cost_for_generating_income/\
    extraction_run_values.json \
  --output-file hallucination_rates.json \
  --top-k 10
\end{lstlisting}
...
- volkswagen_2024 | total_accounts_receivable | rate_non_null=0.1881 (38/202)
...
- alibaba_2025 | total_accounts_receivable | rate_non_null=0.0990 (20/202)
...
calculation, which gives us definite results. The script is part of this
json extraction script that runs automatically after extracting the
relevant fields:
\textbf{llm\_extract\_json\_from\_financial\_statements.py}
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 46956000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 51454000000.0,
...
\textbf{h, i) Applicability to Other Annual Reports}
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 4258000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 4728000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 4731000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 46606000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 48203000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 29362000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 30652000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 107363000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 101223000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 38015000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "short_term_market_securities": 0.0,
    "total_accounts_receivable": 43681000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 88230000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 89985000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 56924000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 69905000000.0,
...
\begin{lstlisting}[style=json]
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 47964000000.0,
...
},
{
  ...
  "field_values": {
    ...
    "total_accounts_receivable": 52340000000.0,
...
\hypertarget{ref:benninga06}{}\hyperlink{ref:benninga06}{[Benninga(06)]} Benninga, Simon, (2006), Principles of Finance with
Excel, Oxford, London.
