...
\item
  ...
  \item
    ...
    modeled as a logit-linear policy ratio of sales, capturing the
    empirical pattern that firms like Apple scale short-term borrowing
    with revenue:
    EffSTDebt\textsubscript{t} = Sales\textsubscript{t} $\cdot$
    $\sigma(\alpha + \beta \cdot t)$, where $\sigma$ is the sigmoid
    function and $\alpha, \beta$ are trainable parameters.
  \end{itemize}
\item
...
financing activities into two separate modules---External Financing
...
Both the total liquidity ratio and the cash allocation evolve over time
via logit-linear trends, with a baseline floor that is independent of
sales:
...
\hyperlink{ref:pareja09}{Pareja(09)}, as we have explained in the \emph{Balance Sheet
Elements} subsection. Also, there is a limitation of assuming a liquidity balance
...
OpEx + tax anomalies): one-step-ahead fit of the trained recovery
model against the synthetic ground truth it was trained on.  Black
markers are the synthetic observations produced by the generator model;
red markers are the trained model's \texttt{forecast\_step}
predictions, initialized from each preceding state.  The 2020 spike and
2022 dip in the Tax panel correspond to the known one-time anomalies
$\delta_{2020}=\$2\text{B}$ and $\delta_{2022}=-\$0.5\text{B}$, which
the \texttt{TaxWithAnomalies} module reproduces exactly.  The slight
noise in the OpEx panel reflects the injected aleatoric noise
